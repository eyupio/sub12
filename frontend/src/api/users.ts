...
export const usersApi = {
  ...
  bulkUnfollow: (userIds: string[]) =>
    api.post<{ unfollowed: number }>(`/users/me/unfollow-batch`, { user_ids: userIds }),
  updateMe: (input: UpdateProfileInput) => api.patch<{ id: string; email: string; display_name: string; bio?: string; location?: string; club?: string; avatar_url?: string; profile_visibility: string; default_score_visibility: string; feed_opt_out: boolean; show_follower_counts: boolean; default_distance_unit: string; default_measurement_unit: string; date_format: string; time_format: string; timezone: string }>('/users/me', input),
