...
function ConfirmDeleteModal({ club, onConfirm, onCancel, isPending }: {
  club: Club
  onConfirm: () => void
  onCancel: () => void
  isPending: boolean
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-sm bg-card border border-subtle rounded-t-2xl sm:rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="t-section-title">Delete Club</h2>
          <button onClick={onCancel} className="text-muted hover:text-secondary transition-colors">
            <X size={18} />
          </button>
        </div>
        <p className="text-sm text-secondary">
          Permanently delete <strong className="text-primary">{club.name}</strong>? This cannot be undone.
        </p>
...
export default function AdminClubDetail() {
  ...
  const { data: membersData } = useQuery({
    ...
    queryFn: () => adminClubsApi.listMembers(id),
  })
  ...
  const saveMutation = useMutation({
    mutationFn: () => adminClubsApi.update(id, {
  ...
  const removeMemberMutation = useMutation({
    mutationFn: (userId: string) => adminClubsApi.removeMember(id, userId),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => adminClubsApi.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-clubs'] })
  ...
  if (error || !club) {
    return (
      <div className="max-w-2xl mx-auto px-4 lg:px-6 py-6 lg:py-8">
        <p className="text-[var(--error-text)] text-sm">{error ? parseError(error) : 'Club not found.'}</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 lg:px-6 py-6 lg:py-8 space-y-6">
      ...
      <div>
        <h1 className="t-page-title">{club.name}</h1>
