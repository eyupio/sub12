...
import { notificationsApi, type NotificationPreferences, type NotificationPreferencesPatch } from '../api/notifications'
...
function NotificationsTab() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['notification-prefs'], queryFn: () => notificationsApi.getPreferences() })
  ...
  return (
    <div className="space-y-5">
      <div className="lc-section">
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-muted">Choose how you want to be notified for each event. In-app shows in the bell menu; email is sent to your account address.</p>
          ...
          {!isLoading && data && (
            <>
                    ...
                    <li key={row.inAppKey} className="flex items-start justify-between gap-3 p-3 rounded border border-subtle bg-surface">
                      ...
                      <div className="shrink-0 flex items-start gap-4 sm:gap-6">
                        <label className="flex flex-col items-center gap-1 text-[10px] uppercase tracking-wider text-muted cursor-pointer w-12">
                          ...
                          <input type="checkbox" checked={inAppVal} onChange={(e) => mutation.mutate({ [row.inAppKey]: e.target.checked } as NotificationPreferencesPatch)} aria-label={`${row.label} in-app`} className="scale-125" />
                        </label>
                        <label className="flex flex-col items-center gap-1 text-[10px] uppercase tracking-wider text-muted cursor-pointer w-12">
                          <span className="sm:hidden">Email</span>
                          <input type="checkbox" checked={emailVal} onChange={(e) => mutation.mutate({ [row.emailKey]: e.target.checked } as NotificationPreferencesPatch)} aria-label={`${row.label} email`} className="scale-125" />
                        </label>
                      </div>
                    </li>
                  )
              ...
              <div className="pt-2 space-y-2">
                <h2 className="t-section-title">Moderation</h2>
                <li className="list-none flex items-start justify-between gap-3 p-3 rounded border border-subtle bg-surface">
                  <div className="flex-1 min-w-0">
                  ...
                  <label className="shrink-0 cursor-pointer">
                    <input type="checkbox" checked={data.report_filed} onChange={(e) => mutation.mutate({ report_filed: e.target.checked })} aria-label="Reports filed" className="scale-125" />
                  </label>
                </li>
              </div>

              <div className="pt-2 space-y-2">
                <h2 className="t-section-title">Email digests</h2>
                <li className="list-none flex items-start justify-between gap-3 p-3 rounded border border-subtle bg-surface">
                  <div className="flex-1 min-w-0">
                  ...
                  <label className="shrink-0 cursor-pointer">
                    <input type="checkbox" checked={data.digest_email} onChange={(e) => mutation.mutate({ digest_email: e.target.checked })} aria-label="Digest email" className="scale-125" />
                  </label>
                </li>
              </div>
            </>
