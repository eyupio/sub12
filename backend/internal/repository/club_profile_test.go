...
func clubTestPool(t *testing.T) (*pgxpool.Pool, func()) {
	t.Helper()
...
// newClubTestUser inserts a throwaway user and registers its cleanup.
...
func TestClubProfileRoundTrip(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { pool.Exec(context.Background(), `DELETE FROM clubs WHERE id = $1`, club.ID) })
...
func TestClubDirectoryFilters(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() {
		pool.Exec(context.Background(), `DELETE FROM clubs WHERE id = ANY($1)`, []string{near.ID, far.ID})
	})
