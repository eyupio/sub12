...
const locationCols = `id, user_id, name, address, lat, lng, default_distance_m, default_distance_unit, default_target_preset, notes, created_at, updated_at`
...
func (r *LocationRepository) Create(ctx context.Context, userID string, in *model.CreateLocationInput) (*model.Location, error) {
	loc, err := scanLocation(r.db.QueryRow(ctx, `
		INSERT INTO locations (user_id, name, address, lat, lng, default_distance_m, default_distance_unit, default_target_preset, notes)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
		RETURNING `+locationCols,
		userID, in.Name, in.Address, in.Lat, in.Lng,
		in.DefaultDistanceM, in.DefaultDistanceUnit, in.DefaultTargetPreset, in.Notes,
	))
	if err != nil {
