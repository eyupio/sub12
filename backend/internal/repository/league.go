...
func (r *LeagueRepository) Create(ctx context.Context, userID string, input *model.CreateLeagueInput) (*model.League, error) {
	...
	`, input.Name, input.Description, input.Type, userID, input.ClubID).Scan(
		...
		&league.Type, &league.ImageURL, &league.ClubID, &league.CreatedBy, &league.CreatedAt,
	)
	if err != nil {
...
func (r *LeagueRepository) ListPublic(ctx context.Context) ([]*model.League, error) {
	rows, err := r.db.Query(ctx, `
		SELECT l.id, l.name, l.description, l.type::text, l.image_url, l.club_id, l.created_by, l.created_at,
	...
	for rows.Next() {
		var l model.League
		if err := rows.Scan(&l.ID, &l.Name, &l.Description, &l.Type, &l.ImageURL, &l.ClubID, &l.CreatedBy, &l.CreatedAt, &l.MemberCount); err != nil {
...
func (r *LeagueRepository) GetByID(ctx context.Context, id string) (*model.League, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT l.id, l.name, l.description, l.type::text, l.join_code, l.image_url, l.club_id, l.created_by, l.created_at,
		       COUNT(lm.user_id) AS member_count
		FROM leagues l
		LEFT JOIN league_members lm ON lm.league_id = l.id
		WHERE l.id = $1
		GROUP BY l.id
	`, id).Scan(&l.ID, &l.Name, &l.Description, &l.Type, &l.JoinCode, &l.ImageURL, &l.ClubID, &l.CreatedBy, &l.CreatedAt, &l.MemberCount)
	if err != nil {
