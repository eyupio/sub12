...
func (r *SocialRepository) GetPublicProfile(ctx context.Context, userID string) (*model.PublicProfile, error) {
	...
	err := r.db.QueryRow(ctx,
		`SELECT id, display_name, bio, location, club, avatar_url, profile_visibility, show_follower_counts, star_level, created_at
		 FROM users WHERE id = $1`,
		userID,
	).Scan(&p.ID, &p.DisplayName, &p.Bio, &p.Location, &p.Club, &p.AvatarURL, &p.ProfileVisibility, &p.ShowFollowerCounts, &p.StarLevel, &p.CreatedAt)
	if err != nil {
